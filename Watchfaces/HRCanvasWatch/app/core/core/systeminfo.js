...
define({
    ...
    def: function coreSystemInfo(e, tizen) {
        ...
        /**
         * Add listener for battery change 
         * @memberof core/systeminfo
         * @fires "battery.low"
         */
        function listenBatteryChange() {
            systeminfo.addPropertyValueChangeListener(
                'BATTERY',
                function change(battery) {
                    if (battery.isCharging) {
                        e.fire('battery.isCharging');
                    }
                    else   {
                        e.fire('battery.notCharging');
                    }
                    setLevel(battery);
                   
                    e.fire('battery.change',getLevel());
                }
            );
        }
        function checkBattery() {
            systeminfo.getPropertyValue('BATTERY', function getCurrentValue(battery){
            	setLevel(battery);
            	e.fire('battery.change',getLevel());
            });
        }
        ...
        return {
            ...
            listenBatteryChange: listenBatteryChange,
